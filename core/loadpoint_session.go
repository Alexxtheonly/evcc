...
func (lp *Loadpoint) stopSession() {
	...
	// abort the session if charging has never started - Connected is set on
	// the in-memory session as soon as the vehicle plugs in, but is never
	// persisted for a session that aborts here: a plug-in that never led to a
	// charge (declined by a schedule, cost limit, or the user) leaves no trace
	// at all. Deliberately left as is - changing it would affect every
	// consumer of session history, not just the ledger. Known gap: "it could
	// have charged and didn't" is exactly the case with no record.
	if s.Created.IsZero() {
		return
	}
