...
type Loadpoint struct {
	...
	connectAtBoot  bool             // evVehicleConnect fired because a car was already plugged in at startup, not because it just plugged in
...
func (lp *Loadpoint) evVehicleConnectHandler() {
	...
	// Skip this on the first poll after a restart with a car already plugged
	// in - evcc's start time is not the plug-in time, and stamping it as one
	// would be indistinguishable from a real connect at that moment. The
	// information genuinely isn't available, so leave it unset rather than
	// fabricate it.
	if lp.connectAtBoot {
...
func (lp *Loadpoint) updateChargerStatus() (bool, error) {
	...
	for _, status := range statusChanges {
		...
		for _, ev := range statusEvents(prevStatus, status) {
			// a connect event on the first poll after startup (prevStatus
