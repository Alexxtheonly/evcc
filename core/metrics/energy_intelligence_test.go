...
func TestDecisionSnapshotsSurviveNewRunsButNotDeletedEconomics(t *testing.T) {
	...
	require.Equal(t, "optimizer_snapshot_legacy_charge_ceiling_assumed_full", out[0].Outcome.AssumptionsSource)
	...
	require.Equal(t, "historical_snapshot_unavailable_or_assumptions_changed", out[0].Outcome.Reason)
