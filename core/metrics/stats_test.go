...
func TestCollectorEnergyStats(t *testing.T) {
	...
	e := col.entity
	require.NoError(t, persist(e, slotStart.AddDate(0, 0, -8), 100, 0, nil, false, false))               // outside 7d
	require.NoError(t, persist(e, slotStart.Add(-24*time.Hour-15*time.Minute), 5, 0, nil, false, false)) // 7d only
	require.NoError(t, persist(e, slotStart.Add(-24*time.Hour), 7, 0, nil, false, false))                // 24h window start
	require.NoError(t, persist(e, slotStart.Add(-12*time.Hour), 3, 0, nil, false, false))                // yesterday, within 24h
	require.NoError(t, persist(e, midnight, 1, 0, nil, false, false))                                    // first slot today
	require.NoError(t, persist(e, slotStart.Add(-15*time.Minute), 2, 0, nil, false, false))              // last completed slot
	require.NoError(t, persist(e, slotStart, 4, 0, nil, false, false))                                   // current slot, excluded
